...
- GitHub：`main` 已推送至 `https://github.com/taidai/omnithings.git`（commit `71724ed`）
- 关键修复：`backend/app/api/nodes.py` 中 `router = APIRouter()` 与 helper 定义被错放到 endpoint 之后，导致容器启动 `NameError: name 'router' is not defined`。已重写文件，确保定义在前、endpoint 在后。
...
1. 节点树后端 CRUD：
   ...
   - `DELETE /api/v1/nodes/{id}`：递归级联删除子孙节点。
2. 规则模板后端：
...
- `GET /api/v1/health`：status ok，version 0.4.5
