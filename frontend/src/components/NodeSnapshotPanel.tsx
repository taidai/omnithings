...
import { fetchSnapshots, type SnapshotPoint } from '../api/client'

interface NodeSnapshotPanelProps {
  nodeId: string
}

export default function NodeSnapshotPanel({ nodeId }: NodeSnapshotPanelProps) {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      const data = await fetchSnapshots(nodeId, range, page, pageSize)
